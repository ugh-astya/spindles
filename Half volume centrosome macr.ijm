...
for(i=0; i<list.length; i++){ //going through file list
  if(endsWith(list[i],".czi")&& matches(list[i], ".*63x.*")){ //only open .cziand 63x files
	...
	run("ROI Manager...");
	roiManager("Open", "D:/MSThesis/Confocal_Images/Left HALF ROI.roi");//this opens the ROI that covers left half of the image inside which the area will be measure and later volume calculated.
	roiManager("Select", 0);
