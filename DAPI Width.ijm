//Spindle Width Macro
//This macro uses 3 channel images, where each channel has been max-intensity projected
...
for (i = 0; i < list.length; i++){
    ...
    if(endsWith(input, ".tif") && matches(list[i], ".*63x.*")) { //checking if the input file is a .tif file or not
      ...
      run("Fit Polynomial", "polynomial=12 degree=2-40 guess"); //Use the BAR plugin function fit polynomial
      windowname=split(C3,".");
      ...
      middlevalue=getResult("Y1",(nResults/2)); //getting the middle value of the plot table (Value at the DAPI peak)
      
      //if the middle value is greater than the lower traingle threshold, go ahead and save it in the table and find the width
	  ...
	  print(dapiwidth);
      } else {//if the middle value is less than the lower triangle threshold, we should use Mean thresholding to find the width
      ...
      //text = replace(C3, ".tif", "");
      //print(text);
      //selectWindow("Polynomial fit Plot of "+text);
      //profile=getProfile();
      //for (i=0; i<profile.length; i++)
      //setResult("Value", i, profile[i]);
  	  //updateResults;
      //IJ.renameResults("Plot Values","Results");
      //saveAs("Results", output+imageName+"_DAPI-Plot.csv");
